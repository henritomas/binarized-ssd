...
from tensorflow.keras.layers import Input, Lambda, Conv2D, DepthwiseConv2D, MaxPooling2D, BatchNormalization, ELU, Reshape, Concatenate, Activation, Add
...
def mobilenet(input_tensor, alpha=1.0, depth_multiplier=1):
    ...
    skip_6 = x
    x = _depthwise_conv_block_classification(skip_6, 512, alpha, depth_multiplier, block_id=7)
    skip_7 = Add()([x, skip_6])
    x = _depthwise_conv_block_classification(skip_7, 512, alpha, depth_multiplier, block_id=8)
    skip_8 = Add()([x, skip_7])
    x = _depthwise_conv_block_classification(skip_8, 512, alpha, depth_multiplier, block_id=9)
    skip_9 = Add()([x, skip_8])
    x = _depthwise_conv_block_classification(skip_9, 512, alpha, depth_multiplier, block_id=10)
    skip_10 = Add()([x, skip_9])
    x = _depthwise_conv_block_classification(skip_10, 512, alpha, depth_multiplier, block_id=11) #11 conv4_3 (300x300)-> 19x19 
    conv4_3 = Add()([x, skip_10])

    x = _depthwise_conv_block_classification(conv4_3, 1024, alpha, depth_multiplier, strides=(2, 2), block_id=12)   # (300x300) -> 10x10 
    skip_12 = x
    x = _depthwise_conv_block_classification(skip_12, 1024, alpha, depth_multiplier, block_id=13) # 13 fc7 (300x300) -> 10x10
    fc7 = Add()([x, skip_12])
